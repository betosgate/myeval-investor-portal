...
export default function MacroEconomyPage() {
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-5 gap-3">{macroIndicators.slice(0, 5).map((m) => <MetricCard key={m.indicator} label={m.indicator} value={m.value} subtitle={m.detail} icon={DollarSign} />)}</div>
      <div className="grid grid-cols-5 gap-3">{macroIndicators.slice(5).map((m) => <MetricCard key={m.indicator} label={m.indicator} value={m.value} subtitle={m.detail} icon={TrendingUp} />)}</div>
      <PhilippinesCharts />
      ...
      <Card><CardHeader><CardTitle>Credit Ratings</CardTitle></CardHeader><CardContent><div className="grid grid-cols-3 gap-4">{[{a:"S&P",r:"BBB+",o:"Stable"},{a:"Moody's",r:"Baa2",o:"Stable"},{a:"Fitch",r:"BBB",o:"Stable"}].map((c)=>(<div key={c.a} className="bg-primary/5 rounded-lg p-3 border text-center"><p className="text-xs text-muted-foreground">{c.a}</p><p className="text-xl font-mono font-bold">{c.r}</p><p className="text-xs text-muted-foreground">{c.o}</p></div>))}</div></CardContent></Card>
